var bird = {
    ...
    fallSpeed: 0,//小鸟下落的速度
    flyBird: function() {//控制飞翔下落额函数
        bird.flyTimer = setInterval(fly, 40)
        function fly() {
            bird.div.style.top = bird.div.offsetTop + bird.fallSpeed++ +"px";
            if(bird.div.offsetTop < 0) {
                bird.fallSpeed = 2; // 这里的作用是用于控制小鸟不要飞出界面
            }
            if(bird.div.offsetTop >= 395) {
                bird.fallSpeed = 0;
                clearInterval(bird.flyTimer) // 掉到地面，清楚飞翔的定时器
                clearInterval(bird.wingTimer)  // 掉到地面， 清楚翅膀摆动的定时器
            }
    ...
    wingWave: function(){ //控制小鸟翅膀煽动
        var down = ["url(img/up_bird0.png)","url(img/up_bird1.png"]
        var up = ["url(img/down_bird0.png)","url(img/down_bird1.png"]
        var i = 0, j = 0;
        function wing() {
            if (bird.fallSpeed > 0) {
